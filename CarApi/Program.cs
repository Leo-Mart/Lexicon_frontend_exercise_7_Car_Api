...
// =========================================================
#region
builder.Services.AddDbContext<VehicleContext>(options => options.UseSqlite("Data Source=cars.db"));
#endregion
...
app.MapGet("/api/cars", async (VehicleContext db) => Results.Ok(await db.Cars.ToListAsync()));

// READ ONE (GET /api/cars/{id})
app.MapGet(
    "/api/cars/{id:int}",
    async (int id, VehicleContext db) =>
...
app.MapPost(
    "/api/cars",
    async (Car bil, VehicleContext db) =>
    {
        db.Cars.Add(bil);
...
app.MapPut(
    "/api/cars/{id:int}",
    async (int id, Car bilInput, VehicleContext db) =>
    {
        var car = await db.Cars.FindAsync(id);
        if (car is null)
            return Results.NotFound($"Bil med ID {id} hittades inte.");
